...
class FaceData :
    ...
    def __init__(self) -> None:
        ...
        self.normalized_fkp_coords = []             # 人脸关键点地标（归一化坐标）
        self.box_coords = []                        # 人脸边界框地标（原始比例坐标）
        self.fkp_coords = []                        # 人脸关键点地标（原始比例坐标）
    ...
    def __repr__(self) :
        return '\n'.join([
            ...
            self._print_fkp_coords(self.normalized_fkp_coords, True), 
            self._print_box_coords(self.box_coords, False), 
            self._print_fkp_coords(self.fkp_coords, False), 
        ])
    ...
    def _print_fkp_coords(self, fkp_coords, normalized) :
        desc = "（归一化）" if normalized else ''
        info = []
        if len(fkp_coords) == 6 :
            info.append(f'[人脸关键点地标{desc}]: ')
